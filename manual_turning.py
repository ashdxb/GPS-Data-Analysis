...
for participant_id in normal_participants_list:
    ...
    for path_num in range(0, 3):
        ...
        df = add_cols(df)
        # print(f"Diff Milliseconds: {df['diff_milliseconds'].mean()}")
        # print(f"Diff Seconds: {df['diff_seconds'].mean()}")
    ...
    if only_407:
        break
# print(pd.DataFrame(collect_all_turns))
new_collect = []
