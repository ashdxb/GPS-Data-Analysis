...
for participant_id in [407,408]+[i for i in range(2101,2110)]+[2111]:
    ...
    # read arg[0] as person csv file
        ...
        X_human = np.array([[df['GPS1 (Lat.) [deg]'].values[i], df['GPS1 (Long.) [deg]'].values[i]] for i in range(len(df['GPS1 (Lat.) [deg]']))])
        X_robot = np.array([[df['GPS2 (Lat.) [deg]'].values[i], df['GPS2 (Long.) [deg]'].values[i]] for i in range(len(df['GPS2 (Lat.) [deg]']))])
        
        # # smoothing
        # def smooth(X, window_len=10):
        #     X_smooth = np.zeros(X.shape)
        #     for i in range(X.shape[1]):
        #         X_smooth[:,i] = np.convolve(X[:,i], np.ones(window_len)/window_len, mode='valid')
        #     return X_smooth
        # X_human = smooth(X_human)
        # X_robot = smooth(X_robot)
        def find_turns(X):
            ...
            def PCA_error(X):
                ...
                return np.sqrt(np.mean(np.sum((X - X_projected)**2, axis=1)))
            # err_thres = 2.9/10000000
            err_thres = 2.9/10000000
            time_thres = 7
            def binary_search_PCA_error_minimizer(X, start, end):
                if end - start < time_thres:
                    return -1
                err = PCA_error(X[start:end])
                if err < err_thres:
                    return end
                else:
                    mid = (start + end)//2
                    return binary_search_PCA_error_minimizer(X, start, mid)
            curr = 0
            mark_region = time_thres
            turns = [False for _ in range(len(X))]
            while curr < len(X):
                next = binary_search_PCA_error_minimizer(X, curr, len(X))
                if next == -1:
                    if curr <len(X)-mark_region:
                        turns[curr:curr+mark_region] = [True for _ in range(mark_region)]
                    else:
                        turns[curr:] = [True for _ in range(len(X)-curr)]
                    curr += mark_region
                    # turns[curr] = True
                    # curr += 1
                else:
                    curr = next
            return turns
        turns_human = find_turns(X_human)
        turns_robot = find_turns(X_robot)
        df['is_turning'] = turns_human
        df['is_turning_robot'] = turns_robot
        ...
        # if avgs[-1] != avg_dir:
        #     avgs.append(avg_dir)
        # df['walking_direction_encoded'] = [avgs[clustering[i]] for i in range(len(clustering))]
        ...
        # df['walking_direction_derivative'] = df['walking_direction_derivative'].rolling(10).mean()
