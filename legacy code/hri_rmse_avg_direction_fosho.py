...
for participant_id in [407,408]+[i for i in range(2101,2110)]+[2111]:
    ...
    # read arg[0] as person csv file
        ...
        pre_alter = parent_dir + f'/results/{participant_id}/' + pre + '_' + str(i+1) + '_'
        ...
        if not os.path.exists(parent_dir + '/results/'):
            ...
            os.makedirs(parent_dir + '/results/')
        if not os.path.exists(parent_dir + f'/results/{participant_id}/'):
            os.makedirs(parent_dir + f'/results/{participant_id}/')
        ...
        for i in range(1, len(walking_direction_encoded)-1):
            if is_turning[i] == 1 and not turning:
                ...
                if turning:
                    ...
                    if curr_n < n_limit:
                        curr_avg = (curr_avg*curr_n + walking_direction_encoded[i])/(curr_n+1)
                        curr_n += 1
                        last_avg = walking_direction_encoded[i]
                    else:
                        curr_avg = (curr_avg*n_limit - last_avg + walking_direction_encoded[i])/n_limit
        avgs_between.append(curr_avg%360)
        print(avgs_between)

        for i in range(1, len(avgs_between)):
            if abs(avgs_between[i] - avgs_between[i-1]) < thres:
        ...
        avgs_between_robot = []
        starting_indices_robot = [0]
        curr_avg = walking_direction_encoded_robot[0]
        last_avg = walking_direction_encoded_robot[0]
        curr_n = 1
        # n_limit = 40
        turning = False
        for i in range(1, len(walking_direction_encoded_robot)-1):
            if is_turning_robot[i] == 1 and not turning:
                avgs_between_robot.append(curr_avg%360)
                turning = True
            elif is_turning_robot[i] != 1:
                if turning:
                    curr_avg = walking_direction_encoded_robot[i]
                    last_avg = walking_direction_encoded_robot[i]
                    starting_indices_robot.append(i)
                    curr_n = 1
                    turning = False
                else:
                    if curr_n < n_limit:
                        curr_avg = (curr_avg*curr_n + walking_direction_encoded_robot[i])/(curr_n+1)
                        curr_n += 1
                        last_avg = walking_direction_encoded_robot[i]
                    else:
                        curr_avg = (curr_avg*n_limit - last_avg + walking_direction_encoded_robot[i])/n_limit
        avgs_between_robot.append(curr_avg%360)
        print(avgs_between_robot)

        for i in range(1, len(avgs_between_robot)):
            if abs(avgs_between_robot[i] - avgs_between_robot[i-1]) < thres:
                print("*DEL", avgs_between_robot[i-1], avgs_between_robot[i], df['seconds'][starting_indices_robot[i]], starting_indices_robot[i])
                is_turning_robot[starting_indices_robot[i-1]:starting_indices_robot[i]] = [0]*(starting_indices_robot[i]-starting_indices_robot[i-1])
            else:
                print(avgs_between_robot[i-1], avgs_between_robot[i], df['seconds'][starting_indices_robot[i]], starting_indices_robot[i])
        df['is_turning_robot'] = is_turning_robot
        ...
        # pre = f"../../first_turns"+"/"+pre.split("/")[-1]
        # print(pre)
        # if not os.path.exists(f"/first_turns/"):
        #     os.makedirs(f"/first_turns/")
        pre = pre_alter
        ...
        if len(lefts) < 0 or len(rights) < 0:
            continue
        first_turn_indices_person = list(range(lefts[0], rights[0]+1))
        df['first_turn_person'] = [i in first_turn_indices_person for i in range(n)]
        # print(f"seconds: {df['seconds'][:first_turn_indices_person[-1]+10]}")
        # print(f"seconds: {df.reset_index()['seconds'][:first_turn_indices_person[-1]+10]}")
        # drop current index and reset index
        
        # print(df[10])
        # print(df.iloc[n-1])
        # print("First turn indices seconds: ", df['seconds'][first_turn_indices_person].to_list())
        print("First turn person seconds: ", df['seconds'][df['first_turn_person']].to_list())
        if df['seconds'][first_turn_indices_person].to_list() != df['seconds'][df['first_turn_person']].to_list():
            print("ERROR")
        plt.clf()
        plt.plot(df.where(df['first_turn_person'])['GPS1 (Long.) [deg]'], df.where(df['first_turn_person'])['GPS1 (Lat.) [deg]'], color='coral', label='participant_turning')
        plt.plot(df.where(df['first_turn_person'])['GPS2 (Long.) [deg]'], df.where(df['first_turn_person'])['GPS2 (Lat.) [deg]'], color='green', label='robot_during')
        start_thres = 1
        black_indices = [i for i in first_turn_indices_person if i > start_thres + first_turn_indices_person[0] and i < first_turn_indices_person[-1] and i%10 in [0,1]]
        grey_indices = [i for i in first_turn_indices_person if i > start_thres + first_turn_indices_person[0] and i < first_turn_indices_person[-1] and i%10 not in [0,1] and i%5 in [0,1]]
        df['is_black_first_indices'] = [i in black_indices for i in range(n)]
        df['is_grey_first_indices'] = [i in grey_indices for i in range(n)]
        print("Black indices seconds: ", df['seconds'][df['is_black_first_indices']].to_list())
        print("Grey indices seconds: ", df['seconds'][df['is_grey_first_indices']].to_list())
        plt.plot(df.where(df['is_black_first_indices'])['GPS1 (Long.) [deg]'], df.where(df['is_black_first_indices'])['GPS1 (Lat.) [deg]'], color='black')
        plt.plot(df.where(df['is_grey_first_indices'])['GPS1 (Long.) [deg]'], df.where(df['is_grey_first_indices'])['GPS1 (Lat.) [deg]'], color='brown')
        plt.plot(df.where(df['is_black_first_indices'])['GPS2 (Long.) [deg]'], df.where(df['is_black_first_indices'])['GPS2 (Lat.) [deg]'], color='black')
        plt.plot(df.where(df['is_grey_first_indices'])['GPS2 (Long.) [deg]'], df.where(df['is_grey_first_indices'])['GPS2 (Lat.) [deg]'], color='brown')
        plt.plot(df['GPS1 (Long.) [deg]'][first_turn_indices_person[-1]-start_thres:first_turn_indices_person[-1]+1], df['GPS1 (Lat.) [deg]'][first_turn_indices_person[-1]-start_thres:first_turn_indices_person[-1]+1], color='red')
        plt.plot(df['GPS2 (Long.) [deg]'][first_turn_indices_person[-1]-start_thres:first_turn_indices_person[-1]+1], df['GPS2 (Lat.) [deg]'][first_turn_indices_person[-1]-start_thres:first_turn_indices_person[-1]+1], color='red')
        plt.xlabel('Long. [deg]', fontsize=8)  # Adjust the font size
        plt.ylabel('Lat. [deg]', fontsize=8)   # Adjust the font size
        plt.title('First Turn Map')
        plt.grid(True, which='both', linestyle=':', linewidth=0.5, color='lightgray')
        plt.legend()
        plt.savefig(f'{pre}first_turn_paths.png')

        # plot distance between participant and robot over the first turn
        plt.clf()
        plt.plot(df['seconds'][df['first_turn_person']], df['distance'][first_turn_indices_person], color='coral', label='distance')
        plt.xlabel('Time (s)', fontsize=8)  # Adjust the font size
        plt.ylabel('Distance (m)', fontsize=8)   # Adjust the font size
        plt.title('Distance Over First Turn')
        plt.grid(True, which='both', linestyle=':', linewidth=0.5, color='lightgray')
        plt.legend()
        plt.savefig(f'{pre}first_turn_distance.png')
        
        print("Mean distance over first turn: ", np.mean(df['distance'][first_turn_indices_person]))

        # plot speed difference between participant and robot over the first turn
        plt.clf()
        plt.plot(df['seconds'][df['first_turn_person']], df['speed_difference'][first_turn_indices_person], color='coral', label='speed_difference')
        plt.xlabel('Time (s)', fontsize=8)  # Adjust the font size
        plt.ylabel('Speed Difference (m/s)', fontsize=8)   # Adjust the font size
        plt.title('Speed Difference Over First Turn')
        plt.grid(True, which='both', linestyle=':', linewidth=0.5, color='lightgray')
        plt.legend()
        # plt.savefig(f'{pre}first_turn_speed_difference.png')

        print("Mean speed difference over first turn: ", np.mean(df['speed_difference'][first_turn_indices_person]))
        
        # plot speed difference between participant and robot over the first turn
        plt.clf()
        plt.plot(df['seconds'][df['first_turn_person']], df['GPS1 (2D) [m/s]'][first_turn_indices_person], color='coral', label='speed_person')
        plt.plot(df['seconds'][df['first_turn_person']], df['GPS2 (2D) [m/s]'][first_turn_indices_person], color='green', label='speed_robot')
        plt.xlabel('Time (s)', fontsize=8)  # Adjust the font size
        plt.ylabel('Speed (m/s)', fontsize=8)   # Adjust the font size
        plt.title('Speed Over First Turn')
        plt.grid(True, which='both', linestyle=':', linewidth=0.5, color='lightgray')
        plt.legend()
        plt.savefig(f'{pre}first_turn_speeds.png')
        ...
        # print(walking_direction_difference)
        for i in range(1, len(walking_direction_difference)):
            if walking_direction_difference[i] > 180:
                # print(walking_direction_difference[i], walking_direction[i], walking_direction_robot[i])
        ...
        # plot walking direction difference over the first turn
        plt.clf()
        plt.plot(df['seconds'][df['first_turn_person']], df['walking_direction_difference'][first_turn_indices_person], color='coral', label='walking_direction_difference')
        plt.xlabel('Time (s)', fontsize=8)  # Adjust the font size
        plt.ylabel('Walking Direction Difference (deg)', fontsize=8)   # Adjust the font size
        plt.title('Walking Direction Difference Over First Turn')
        plt.grid(True, which='both', linestyle=':', linewidth=0.5, color='lightgray')
        plt.legend()
        # plt.savefig(f'{pre}first_turn_walking_direction_difference.png')
        
        plt.clf()
        plt.plot(df['seconds'][df['first_turn_person']], df['walking_direction'][first_turn_indices_person], color='coral', label='walking_direction_person')
        plt.plot(df['seconds'][df['first_turn_person']], df['walking_direction_robot'][first_turn_indices_person], color='green', label='walking_direction_robot')
        plt.xlabel('Time (s)', fontsize=8)  # Adjust the font size
        plt.ylabel('Walking Direction (deg)', fontsize=8)   # Adjust the font size
        plt.title('Walking Direction Over First Turn')
        plt.grid(True, which='both', linestyle=':', linewidth=0.5, color='lightgray')
        plt.legend()
        plt.savefig(f'{pre}first_turn_walking_directions.png')

        print("Mean walking direction difference over first turn: ", np.mean(df['walking_direction_difference'][first_turn_indices_person]))
