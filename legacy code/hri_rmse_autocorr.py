...
for participant_id in [407,408]+[i for i in range(2101,2110)]+[2111]:
    ...
    # read arg[0] as person csv file
        ...
        rol = 5
        df['walking_direction_autocorr'] = df['walking_direction'].rolling(rol).apply(lambda x: x.var())
        df['walking_direction_autocorr_robot'] = df['walking_direction_robot'].rolling(rol).apply(lambda x: x.var())
        ...
        # df['walking_direction_derivative_robot'] = df['walking_direction_derivative_robot'].rolling(10).mean()
        
        # plot walking direction derivatives of participant and robot
        line_plot(df, 'seconds', ['walking_direction_autocorr', 'walking_direction_autocorr_robot'], 'Walking Direction Autocorr', 'Time (s)', 'Walking Direction Autocorr', f'{pre}walking_direction_autocorr.png')
        
        thres = 0.4
        # calculate turning
        df['is_turning'] = df['walking_direction_autocorr'].apply(lambda x: 1 if abs(x) > thres else 0)
        df['is_turning_robot'] = df['walking_direction_autocorr_robot'].apply(lambda x: 1 if abs(x) > thres else 0)
